...
from ppref.preferences.special import PartitionedWithMissing, PartitionedPreferences
from ppref.profile_solvers.solver_by_voter_pruning import sequential_solver_of_voter_pruning, sequential_baseline_solver
...
def generate_profile_df(m, n, num_buckets, batch):
    ...
    for i in range(n):
        pp = PartitionedPreferences.generate_a_random_instance(m, num_buckets, rng)
        df.loc[i] = [m, num_buckets, repr(pp)]
...
def run_experiment():
    ...
    with open(out_file, open_mode, buffering=1) as out:
        if open_mode == 'w':
            cols = ['m', 'n', 'num_buckets', 'batch', 'rule', 'winners', 'score_upper', 'score_lower', 'num_pruned_voters',
        ...
        for (m, n, num_buckets, batch) in generate_combs():
            for rule in rule_list:
                ...
                if not is_existing:
                    ...
                    profile: list[PartitionedPreferences] = [eval(pp) for pp in df_in['ppmw']]
                    answer = sequential_solver_of_voter_pruning(profile, get_rule_vector(rule, m))

                    record = [m, n, num_buckets, batch, rule, answer['winners'], answer['score_upper'],
...
def run_experiment_of_baseline():
    sep = '\t'

    rule_list = ['Plurality', '2-approval', 'Borda']

    out_file = get_dir(__file__) / 'experiment_output_of_baseline.tsv'
    open_mode = 'a' if out_file.exists() else 'w'
    with open(out_file, open_mode, buffering=1) as out:
        if open_mode == 'w':
            cols = ['m', 'n', 'num_buckets', 'batch', 'rule', 'winners', 'winner_score', 't_baseline_sec']
            out.write(sep.join(cols) + '\n')
        else:
            df_existing = pd.read_csv(out_file, sep=sep)
        ...
        for (m, n, num_buckets, batch) in generate_combs():
            for rule in rule_list:
                ...
                if not is_existing:
                    ...
                    profile: list[PartitionedPreferences] = [eval(pp) for pp in df_in['ppmw']]
                    answer = sequential_baseline_solver(profile, get_rule_vector(rule, m))

                    record = [m, n, num_buckets, batch, rule, answer['winners'], answer['winner_score'], answer['t_sec']]
                    record = [str(i) for i in record]
