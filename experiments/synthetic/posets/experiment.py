...
def generate_combs():
    ...
    n_list = [100, 1000, 10_000]
    phi_list = [0.1, 0.5, 0.9]
    pmax_list = [0.1, 0.5, 0.9]
    batch_list = list(range(10))

    combs = set()

    # varying phi, fix m, n, pmax
    pmax, n = 0.1, 1000
    for phi in phi_list:
        for batch in batch_list:
            combs.add((m, n, phi, pmax, batch))

    # varying n, fix m, phi, pmax
    pmax, phi = 0.1, 0.5
    for n in n_list:
        for batch in batch_list:
            combs.add((m, n, phi, pmax, batch))

    # varying n, pmax, fix m, phi
    phi = 0.5
    for n in n_list:
...
def run_experiment():
    ...
    with open(out_file, open_mode, buffering=1) as out:
        if open_mode == 'w':
            cols = ['m', 'n', 'phi', 'pmax', 'batch', 'rule', 'winners', 'score_upper', 'score_lower', 'num_pruned_voters',
        ...
        for (m, n, phi, pmax, batch) in generate_combs():
            for rule in rule_list:
                ...
                if not is_existing:
                    ...
                    record = [m, n, f'{phi:.1f}', f'{pmax:.1f}', batch, rule, answer['winners'], answer['score_upper'],
...
def run_experiment_of_baseline():
    ...
    with open(out_file, open_mode, buffering=1) as out:
        if open_mode == 'w':
            cols = ['m', 'n', 'phi', 'pmax', 'batch', 'rule', 'winners', 'winner_score', 't_baseline_sec']
            out.write(sep.join(cols) + '\n')
        else:
            df_existing = pd.read_csv(out_file, sep=sep)
        ...
        for (m, n, phi, pmax, batch) in generate_combs():
            for rule in rule_list:
                ...
                if not is_existing:
                    ...
                    record = [m, n, f'{phi:.1f}', f'{pmax:.1f}', batch, rule,
